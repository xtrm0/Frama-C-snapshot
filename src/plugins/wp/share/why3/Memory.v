...
Definition eqmem {a:Type} {a_WT:WhyType a} (m1:(map.Map.map addr a))
  ...
  (included q 1%Z p a1) -> ((MapExt.get m1 q) = (MapExt.get m2 q)).
...
Definition valid_rw (m:(map.Map.map Z Z)) (p:addr) (n:Z): Prop :=
  ...
  (((offset p) + n)%Z <= (MapExt.get m (base p)))%Z)).
...
Definition invalid (m:(map.Map.map Z Z)) (p:addr) (n:Z): Prop :=
  (0%Z < n)%Z -> (((MapExt.get m (base p)) <= (offset p))%Z \/
  (((offset p) + n)%Z <= 0%Z)%Z).
...
Lemma valid_string : forall (m:(map.Map.map Z Z)), forall (p:addr),
  ...
  ((offset p) < (MapExt.get m (base p)))%Z) -> ((valid_rd m p 1%Z) /\
  ~ (valid_rw m p 1%Z))).
Proof.
  intros m p.
...
Definition framed (m:(map.Map.map addr addr)): Prop := forall (p:addr),
  ((region (base (MapExt.get m p))) <= 0%Z)%Z.
...
Lemma havoc_access : forall {a:Type} {a_WT:WhyType a},
  ...
  (p:addr), forall (a1:Z), ((separated q 1%Z p a1) -> ((MapExt.get (havoc m0
  m1 p a1) q) = (MapExt.get m1 q))) /\ ((~ (separated q 1%Z p a1)) ->
  ((MapExt.get (havoc m0 m1 p a1) q) = (MapExt.get m0 q))).
Proof.
